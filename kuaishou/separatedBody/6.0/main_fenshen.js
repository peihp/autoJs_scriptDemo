...
ui.layout(
    <vertical>
        ...
        <text textColor="#FF0000" marginLeft="5" marginTop="10">版本号： V1.0.9_B2020050102 </text>
        <text textColor="#FF0000" marginLeft="5" marginTop="10">此版本为root版本，手机需root</text>
        <text textColor="#FF0000" marginLeft="5" marginTop="10">启动前一定要给“快快”软件root权限</text>
                        ...
                        try{
                            ...
                            if(!isNoRedPag() ){
                                ...
                                if(className("android.widget.ImageView").id("left_btn").exists()){
                                    ...
                                    if (textContains("手极速版").find()) {
                                        let lsdesc = text("快手极速版(双开助手)").findOnce()
                                        if (lsdesc) {
                                            toastLog("进入应用")
                            ...
                            if(!isNoIncome()){
                                sleep(2000)
                                if(id("comment_container").exists()){
                            ...
                            if (desc("继续邀好友赚钱").findOnce() || desc("版本更新").findOnce()  || desc("立即安装").findOnce()  || desc("下载返利").findOnce()  || text("搜索").findOnce() || desc("切换搜索引擎").findOnce()) {
                                back()
                                sleep(1000)
                                back()
                            }
...
function qiandao() {
    ...
    while (true) {
        try{
            ...
            if (desc("继续邀好友赚钱").findOnce()  || desc("立即安装").findOnce()  || desc("下载返利").findOnce()  || desc("切换搜索引擎").findOnce()) {
                back()
            }
            ...
            if (temparr.length > 0) {
                ...
                try{
                    if (temparr.length > 1) {//undefied
                        ...
                        Tap(temparr[qdfs].bounds().centerX(), temparr[qdfs].bounds().centerY())
                    }else{
...
function idTap(idkongjian, showtext) {
    if (id(idkongjian).findOnce()) {
        ...
        if (lsid) {
            toast(showtext)
            Tap(lsid.bounds().centerX(), lsid.bounds().centerY())
            sleep(2000)
        }
    }
}
...
function sphuadong(str) {
    ...
    if(str.equals("图片滑动")){
        ...
        var hasSwipe = Swipe(500, 1200, 500, 0, swipeTime)
        if(!hasSwipe){
...
function findIncomeIcon(){
    ...
    try{
        ...
        if(redBag){
            ...
            p=images.findColorEquals(screen,"#ffd235",redBagBounds.centerX(),redBagBounds.top,px, py)//黄色
            r=images.findColorEquals(screen,"#f85050",redBagBounds.centerX(),redBagBounds.top,px, py)//红色
            if(p){
                redBagColor = "#ffd235";
                // toastLog("红包黄色")
            }else if(r){
...
function judgeIsNeedUpdate(){
    // threads.start(function() {
        ...
        //服务端apk版本
        var url = "http://27.155.88.170:8765/xhapi/getupdateurl";
...
function saveRunApp() {
    ...
    //运行日志保存本地
    var logFile = '/sdcard/kuaikuai/runLog.txt';
